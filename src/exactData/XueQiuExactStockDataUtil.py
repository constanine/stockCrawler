...
def getStockInfoDataFromXQ(beginTimeStr,endTimeStr):
    ...
    for row in results:
        try:
            ...
            __analsisXQStockDataAndUpdateDb(data)
        except Exception as err:
            print(err)


def __analsisXQStockDataAndUpdateDb(data):
    deteleSqlTmp = '''
        DELETE FROM `stockdata`.`xueqiuexactstockdata` WHERE code='%S'
    ''';
    insertSqlTmp = '''
        INSERT INO  `stockdata`.`xueqiuexactstockdata`
